...
void OpenServerCommand::execute(vector<string> data) {
    double port = I.interpret(data[1]);
    //open server
    this->m_port = port;
  //OpenServerCommand* op = new OpenServerCommand(port);
//    op->execute();
    ...
    m_fd = client_socket;
    connected = true;
  thread* t1 = new thread(&OpenServerCommand::readFromSim, this);
