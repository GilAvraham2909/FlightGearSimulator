...
void Interpreter :: setVariables(vector<string> vars, vector<string> num) {
    vector<string> :: iterator it = num.begin();
...
double Interpreter :: interpret(string str) {
    ...
    while (!postfix.empty()) {
        if (isdouble(postfix.front())) {
            ...
            double val;
           if (varMap.find(postfix.front()) != varMap.end()) {
                val = varMap[postfix.front()]->getVal();
